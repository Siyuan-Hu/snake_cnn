...
# 用一维数组来表示二维的东西
# board表示蛇运动的矩形场地
# 初始化蛇头在(1,1)的地方，第0行，HEIGHT-1行，第0列，WIDTH-1列为围墙，不可用
# 初始蛇长度为1
board = [0] * FIELD_SIZE
snake = [0] * (FIELD_SIZE+1)
snake[HEAD] = 5*WIDTH+5
snake_size = 1
        ...
        for i in range(1,HEIGHT-1):
            for j in range(1,WIDTH-1):
                ...
                if i*WIDTH+j==snake[HEAD]:
                    f.write('2 ')
                    continue
                for knot in range(1, snake_size):
                    if i*WIDTH+j==snake[knot]:
                        f.write('1 ')
                        flag=True
                        break
                if flag==False:
...
while key != 27:
    if snake_size>20:
        ...
        for i in range(1,WIDTH-1):
            for j in range(1,HEIGHT-1):
                if i*WIDTH+j!=food and i*WIDTH+j!=snake[HEAD]:
                    board[i*WIDTH+j] = UNDEFINED # 蛇尾变为空格
                    win.addch(i, j, ' ')
    win.border(0)
    ...
    #print("matrix")
    #print(inputMatrix)
    out=model.predict(inputMatrix,batch_size=batch_size,verbose=0)

    f=file("hello.data","a+")
    f.write(str(snake[HEAD])+','+str(food))
    f.writelines('\n')
    f.write(str(out[0]))
    ...
    best_move=find_max_dir(out[0][0],out[0][1],out[0][2],out[0][3])
    make_move(best_move)
